...
echo "📋 Extracting Windows Terminal actions and keybindings..."
...
if [ -f "$WINDOWS_SETTINGS_PATH" ]; then
  if command -v jq &> /dev/null; then
    jq '{actions: .actions, keybindings: .keybindings}' "$WINDOWS_SETTINGS_PATH" > "$(pwd)/configs/windows-terminal-settings.json"
    echo "✅ Saved actions and keybindings to configs/windows-terminal-settings.json"
  else
